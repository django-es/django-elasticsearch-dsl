...
from django_opensearch_dsl.exceptions import VariableLookupError
from django_opensearch_dsl.fields import (
    BooleanField,
    ByteField,
    CompletionField,
    DODField,
...
class DODFieldTestCase(TestCase):
    ...
    def test_get_value_from_instance_related_attr(self):
        field = DODField(attr="related.attr1")
    ...
    def test_get_value_from_none(self):
        field = DODField(attr="related.none")
        self.assertEqual(field.get_value_from_instance(None), None)

    def test_get_value_from_instance_ignored(self):
        field = DODField(attr="attr1")
        instance = NonCallableMock(attr1="foo")
        self.assertEqual(field.get_value_from_instance(instance, "foo"), None)

    def test_get_value_from_lazy_object(self):
        field = DODField(attr="translation")
